<template>
    ...
    <div class="submit">
        <v-btn @click.prevent="login" size="small" class="pill">Log In</v-btn>
    </div>
  </form>
</template>
     ...
     async login() {
      ...
      try {
        const response = await axios.post(
          `http://127.0.0.1:8000/user/login/${this.email}`,
          data
        );
        if (response.data.code == 200) {
          try {
            const userResponse = await axios.get(`http://127.0.0.1:8000/user/${this.email}`)
            ...
            if (this.userStore.isLoggedIn) {
              console.log(this.userStore.user.account_type);
              if (this.userStore.user.account_type === 'Buyer') {
            ...
            console.error("There was a problem finding user", error)
            alert(`Error logining in: Please create an account first`)
          }
        }
      }
        catch (error) {
          console.error("There is an issue with authentication", error)
