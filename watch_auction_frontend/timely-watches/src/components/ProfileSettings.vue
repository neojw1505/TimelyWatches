...
export default {
  ...
  methods: {
    async makeChanges() {
      this.userStore.editFirstName(this.firstName)
      this.userStore.editLastName(this.lastName)
      this.userStore.editPassword(this.password)
      const params = {
      ...
      try {
        await axios.put(`http://127.0.0.1:8000/user/${this.email}`, params);
      } catch (error) {
