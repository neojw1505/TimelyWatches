...
<template>
    <v-card flat>
      ...
      <v-data-table :key="componentKey" :search="search" :items="items" :headers="headers" item-key="auction_id">
...
<template v-slot:item.start_time="{ item }">
  ...
  data() {
    return {
      ...
      componentKey: 0,
      headers: [
        { text: 'Delete Auction', value: 'delete'},
  ...
  methods: {
    ...
    async deleteItem(item) {
      const response = await axios.delete(`http://127.0.0.1:5001/auction/${item.auction_id}`);
