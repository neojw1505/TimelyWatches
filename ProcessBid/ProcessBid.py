...
@app.route("/processbid/authbid", methods=["POST"])
def authenticate_bid():
    ...
    # No existing bids, compare with start price to process bid
        ...
        if start_price != None and bid_amount >= start_price:
            # Bid authenticated
            create_bid_response, create_bid_status_code = create_bid(
                auction_id, user_id, bid_amount
            )
...
@app.route("/processbid/getAllAuctionAndUserhighestBidByUser/<int:user_id>")
